/*---------------------------------------------------------------------------------------------  
  ...
  Sending brightness messages from Max to Arduino through Serial. Example: https://youtu.be/68L-WHh3Ows
...
// Sending messages from Max MSP to the arduino via serial
void ReadSerial ()
{
  //read until there is nothing to read, or until we get a newline: newline indicates new data set
  bool dataAligned = false;
  ...
  if (dataAligned)
  {
    //Read the serial data sent from max in order.

    //The first number received is brightness
    LEDBrightness = Serial.read();
  }

  //Clear all data, leaving behind one character in case it is a newline
...
//Sending messages from Max MSP to the ESP8266 via OSC 
void ReadOSC ()
{
